...
from rag_integration import init_rag_service, get_rag_service
...
rag_service = None  # Service RAG global
...
async def startup_event():
    ...
    global ollama_status, rag_status, rag_service, langchain_pipeline, mystical_mode, mystical_processor
    ...
    try:
        ...
        ollama_base_url = config.get("models", {}).get("ollama", {}).get("base_url", "http://100.64.0.34:11434")
        ollama_model = config.get("models", {}).get("ollama", {}).get("model_name", "qwen2.5:1.5b")
    except Exception as e:
        print(f"Erreur lors du chargement de la configuration Ollama : {e}")
        ollama_base_url = "http://100.64.0.34:11434"
        ollama_model = "llama4:latest"

    # Initialiser Ollama avec la config YAML
    try:
        init_ollama_client(base_url=ollama_base_url, model=ollama_model)
    ...
    try:
        global rag_service
        rag_service = init_rag_service(index_type="faiss")  # ou "chroma" selon votre préférence
            ...
            async checkOllamaStatus() {
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (data.available) {
                            this.ollamaStatus.textContent = `✅ Ollama actif - Modèle: ${data.current_model || 'llama'}`;
...
async def generate_smart_response(
    ...
    if rag_service:
        ...
        rag_context_data = rag_service.get_rag_response_context(message)
        context_rag = rag_context_data.get('context', '')
        
        if context_rag and len(context_rag.strip()) > 10:
            sources = rag_context_data.get('sources', [])
            source_files = list(set([s.get('file_name', 'inconnu') for s in sources]))
...
async def chat(request: ChatRequest):
    ...
    # Normalisation anti-None pour les réponses Assistant dans l'API
    if response is None:
        response = ""
