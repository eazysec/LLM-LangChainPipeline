...
class ConversationDB:
    ...
    def __init__(self, db_path: str = None):
        import os
        # Si aucun chemin n'est fourni, utiliser un chemin relatif à la racine du projet
        if db_path is None:
            # Essayer de trouver le répertoire racine du projet
            current_file = os.path.abspath(__file__)
            project_root = os.path.dirname(current_file)
            db_path = os.path.join(project_root, "data", "conversations.db")
        ...
        with self.get_connection() as conn:
            try:
                ...
                cursor = conn.execute("""
                    ...
                    WHERE conversation_search MATCH ? -- Correction: utiliser le nom de la table FTS5, pas l'alias cs
            ...
            results = []
            for row in cursor:
                # TODO: Vérifiez ici que vous ne faites pas .split() sur un objet de type list (par exemple dans l'usage ailleurs du résultat!)
                results.append({
    ...
    def search_relevant_conversations(self, query: str, limit: int = 10) -> List[Dict]:
        """Alias rétrocompatible pour search_conversations (pour anciens appels)"""
